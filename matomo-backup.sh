...
## TAR MATOMO DIR
tar -cf $BACKUPROOT/matomo_dir.tar $MATOMODIR

## SET MATOMO EXTRA TOOLS DB BACKUP PATH
$MATOMODIR/console config:set 'ExtraTools.db_backup_path="$BACKUPROOT"'
...
_EOF_
    ...
    ## REMOVE LOCAL TAR
    rm -v $BACKUPROOT/$BACKUPDIR.tar | tee -a $BACKUPROOT/matomo-backup.log
else
    ...
    mv -v $BACKUPROOT/$BACKUPDIR.tar $LOCALDEST | tee -a $BACKUPROOT/matomo-backup.log
