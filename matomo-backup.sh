...
## CREATE BACKUPDIR
mkdir -p $BACKUPDIRVAR

## CHANGE TO DIR
cd $BACKUPROOT

## TAR MATOMO DIR
tar -cf $BACKUPROOT/$BACKUPDIRVAR/matomo_dir.tar $MATOMODIR

## RUN MATOMO DATABSE BACKUP
$MATOMODIR/console database:backup --backup-path=$BACKUPROOT/$BACKUPDIRVAR

## TAR THE WHOLE BACKUP DIR AND REMOVE DIR IF SUCCESSFUL
tar -cf $BACKUPROOT/$BACKUPDIR.tar $BACKUPROOT/$BACKUPDIRVAR && rm -R $BACKUPROOT/$BACKUPDIRVAR
...
sftp $REMOTESERVER <<_EOF_
    put $BACKUPROOT/$BACKUPDIRVAR.tar $REMOTEDEST/$BACKUPDIRVAR.tar
    bye
_EOF_
...
_EOF_
    ...
    ## REMOVE LOCAL TAR
    rm -v $BACKUPROOT/$BACKUPDIRVAR.tar | tee -a $BACKUPROOT/matomo-backup.log
else
    ...
    mv -v $BACKUPROOT/$BACKUPDIRVAR.tar $LOCALDEST | tee -a $BACKUPROOT/matomo-backup.log
